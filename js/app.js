...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.loadChapter = async (file, updateHistory = true) => {
        // The file variable is already a relative path, so no normalization is needed
        try {
            ...
            if (updateHistory) {
                const title = getTitleFromFilePath(file);
                // Correctly use the GitHub repo name in the URL for history
                const urlForHistory = `/${githubRepo}/${file}`;
                history.pushState({ path: file }, title, urlForHistory);
            }
    ...
    window.loadTOC = async () => {
        try {
            ...
            rootFiles.forEach(item => {
                ...
                link.dataset.filePath = item.file; // Use relative path
            ...
            Object.keys(folderGroups).forEach(folderName => {
                ...
                folderItems.forEach(chapter => {
                    ...
                    link.className = "block px-4 py-2 text-base font-medium text-gray-400 hover:bg-gray-700 hover:text-white rounded-md transition-colors duration-200";
                    link.dataset.filePath = chapter.file; // Use relative path
            ...
            const urlPath = window.location.pathname.replace(`/${githubRepo}/`, ''); // Remove the repo name
